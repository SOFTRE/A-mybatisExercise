package com.xxM;

import com.xxM.dao.UserMapper;
import com.xxM.domain.User;
import org.apache.ibatis.io.Resources;
import org.apache.ibatis.session.SqlSession;
import org.apache.ibatis.session.SqlSessionFactory;
import org.apache.ibatis.session.SqlSessionFactoryBuilder;
import org.junit.After;
import org.junit.Before;
import org.junit.Test;

import java.io.IOException;
import java.io.InputStream;
import java.util.Date;
import java.util.List;

/**
 * @Program: IntelliJ IDEA ssm
 * @Description: this is a testClass
 * @Author: Mr Liu
 * @Creed: Talk is cheap,show me the code
 * @CreateDate: 2019-09-26 17:13:28 周四
 * @LastModifyDate:
 * @LastModifyBy:
 * @Version:
 */

public class MyTest {
    SqlSession sqlSession;
    UserMapper mapperProxy;

    @Before
    public void before() throws IOException {
        //1. 读取SqlMapConfig.xml获得输入流
        InputStream is = Resources.getResourceAsStream("SqlMapConfig.xml");
        //2.创建SqlSessionFactory
        SqlSessionFactoryBuilder builder = new SqlSessionFactoryBuilder();
        SqlSessionFactory sqlSessionFactory = builder.build(is);
        is.close();
        //3. 获得SqlSession
        sqlSession = sqlSessionFactory.openSession();
        //4.通过namespace.id调用
        mapperProxy = sqlSession.getMapper(UserMapper.class);
    }

    @Test
    public void method() {
        List<User> order_by_uid = mapperProxy.findAll("order by uid desc");
        for (User user : order_by_uid) {
            System.out.println(user);
        }
    }

    @Test
    public void method1() {
        User user=new User();
        user.setAddress("ss");
        user.setUsername("zhang");
        user.setSex("男");
        user.setBirthday(new Date());
        mapperProxy.add(user);
    }

    @Test
    public void method2() {
        User user=mapperProxy.findById(2);
        user.setUsername("haha");
        user.setBirthday(new Date());
        user.setSex("男");
        user.setAddress("北京");
        mapperProxy.update(user);
    }

    @Test
    public void method3() {
        mapperProxy.delete(6);
    }

    @Test
    public void method4() {
        List<User> user = mapperProxy.findByFirstName("上");
        System.out.println(user);
    }

    @After
    public void after() {
        sqlSession.commit();
        //6.释放资源
        sqlSession.close();
    }
}
